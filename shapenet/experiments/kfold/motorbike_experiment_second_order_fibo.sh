...
# BRIEF: Experiment with the randomized motorbike dataset                      #
# ---------------------------------------------------------------------------- #
...
VL3DPIPE='/tmp/vl3d_motorbike_stratkfold.json'
# Directory where the point clouds of the dataset are located
DATADIR='/ext4/lidar_data/shapenet/pcloud/Motorbike'
# Output directory for the pipeline
OUTDIR='/ext4/lidar_data/shapenet/vl3d/out/motorbike/order2_fibo/'
# Output file for point cloud with mined features
OUTFEATS='training_motorbike.las'
# Output file to store the trained model
OUTMODEL='rf_motorbike.pipe'
...
TRAINING_DISTRIBUTION='[2000000, 2000000, 2000000, 2000000, 2000000, 0]'
...
# ---   MAIN   --- #
# ---------------- #
# Recopilate motorbike point clouds
find_pclouds
...
for(( it=0 ; it < ${kfolds} ; ++it )); do
    ...
    source training_pipeline_template_second_order_fibo.sh
