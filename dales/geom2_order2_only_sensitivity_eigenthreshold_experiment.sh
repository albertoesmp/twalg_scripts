...
# BRIEF: Script to run the sensitivity analysis on the eigenthreshold          #
#        in the DALES dataset (without height features)                        #
# ---------------------------------------------------------------------------- #
...
num_param_changes=${#EIGENTHRESHOLD[@]}
for (( i=0 ; i < ${num_param_changes} ; ++i )); do


    # ---  MINING JSON : CHANGING EIGENTHRESHOLD  --- #
    # ----------------------------------------------- #
    mining_json="${JSONDIR}/eigen_only2nd_mining_$(( $i + 1 )).json"
    tmp_json="${mining_json}"
    eigenthreshold=${EIGENTHRESHOLD[$i]}
    outdir="${OUTDIR}/sensit/eigen_only2nd_${eigenthreshold}/"
    ...
    "sequential_pipeline": [
    	{
            ...
            "non_degenerate_eigenthreshold": ${eigenthreshold},
...
EOF
    ...
    # ---   TRAINING JSON   --- #
    # ------------------------- #
    train_json="${JSONDIR}/eigen_only2nd_train_$(( $i + 1 )).json"
    tmp_json="${train_json}"
    cat << EOF > "${tmp_json}"
{
    "in_pcloud_concat": [
...
EOF
    ...
    val_json="${JSONDIR}/eigen_only2nd_val_$(( $i + 1 )).json"
    tmp_json="${val_json}"
    cat << EOF > ${tmp_json}
{
...
EOF
    ...
    # ---  SLURM BATCH SCRIPT  --- #
    # ---------------------------- #
    slurm_script="${JSONDIR}/eigen_sensit_only2nd_$(( $i + 1 )).sh"
