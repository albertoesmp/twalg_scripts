...
for (( i=0 ; i < ${num_param_changes} ; ++i )); do
    ...
    # ----------------------------------------------- #
    mining_json="${JSONDIR}/eigen_mining_$(( $i + 1 )).json"
    tmp_json="${mining_json}"
    eigenthreshold=${EIGENTHRESHOLD[$i]}
    outdir="${OUTDIR}/sensit/eigen_${eigenthreshold}/"
    ...
    "sequential_pipeline": [
        ...
        },
        {
            ...
            "decorated_miner": {
                ...
                "frenames": ["floordist_r10", "ceildist_r10"]
            }
        },
        {
            "miner": "FPSDecorated",
            ...
            "decorated_miner": {
                ...
                "outlier_filter": "stdev",
                "fnames": ["floor_distance", "ceil_distance"],
                "frenames": ["floordist_r30", "ceildist_r30"]
            }
        },
        {
...
EOF
    ...
    # ---   TRAINING JSON   --- #
    # ------------------------- #
    train_json="${JSONDIR}/eigen_train_$(( $i + 1 )).json"
    tmp_json="${train_json}"
    cat << EOF > "${tmp_json}"
{
    "in_pcloud_concat": [
...
EOF
    ...
    val_json="${JSONDIR}/eigen_val_$(( $i + 1 )).json"
    tmp_json="${val_json}"
    cat << EOF > ${tmp_json}
{
...
EOF
    ...
    # ---  SLURM BATCH SCRIPT  --- #
    # ---------------------------- #
    slurm_script="${JSONDIR}/eigen_sensit_$(( $i + 1 )).sh"
