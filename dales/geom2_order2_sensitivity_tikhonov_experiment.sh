...
for (( i=0 ; i < ${num_param_changes} ; ++i )); do
    ...
    # --------------------------------------------------- #
    mining_json="${JSONDIR}/tikhonov_mining_$(( $i + 1 )).json"
    tmp_json="${mining_json}"
    tikhonov=${TIKHONOV[$i]}
    outdir="${OUTDIR}/sensit/tikhonov_${tikhonov}/"
...
EOF
    ...
    # ---   TRAINING JSON   --- #
    # ------------------------- #
    train_json="${JSONDIR}/tikhonov_train_$(( $i + 1 )).json"
    tmp_json="${train_json}"
    cat << EOF > "${tmp_json}"
{
    "in_pcloud_concat": [
...
EOF
    ...
    val_json="${JSONDIR}/tikhonov_val_$(( $i + 1 )).json"
    tmp_json="${val_json}"
    cat << EOF > ${tmp_json}
{
...
EOF
    ...
    # ---  SLURM BATCH SCRIPT  --- #
    # ---------------------------- #
    slurm_script="${JSONDIR}/tikhonov_sensit_$(( $i + 1 )).sh"
